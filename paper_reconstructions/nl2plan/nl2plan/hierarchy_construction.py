...
class HierarchyConstruction:
    ...
    def hierarchy_construction(
        ...
        while not no_feedback and i <= max_feedback_retries:
            ...
            while not valid and max_validation_retries > 0:
                type_hierarchy, llm_output, validation_info = self.domain_builder.formalize_type_hierarchy(
                    model=model,
    ...
    def generate_feedback_revision_prompt(
        ...
        prompt = load_file("paper_reconstructions/nl2plan/prompts/hierarchy_construction/feedback_revision.txt")
        prompt = (
            prompt
