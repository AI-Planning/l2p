...
from l2p.validators.base import (
    ValidationResult, SyntaxValidator, FunctionalRule, ValidationRule, 
    _extract_symbols, _verify_symbols, get_ordinal, PDDL_KEYWORDS)
...
def _check_variables(item: Any, allowed_vars: Set[str], result: ValidationResult, location_desc: str):
...
@domain_rule(targets=[PDDLType, Constant, Predicate, Function, DerivedPredicate, 
...
def check_type_inheritance(target: BaseModel, context: Dict[str, Any]) -> ValidationResult:
...
def check_constant_inheritance(target: BaseModel, context: Dict[str, Any]) -> ValidationResult:
...
@domain_rule(targets=[Predicate, Function, DerivedPredicate, Action, DurativeAction, Event, Process])
def check_parameter_types(target: BaseModel, context: Dict[str, Any]) -> ValidationResult:
...
def check_derived_predicate(target: BaseModel, context: Dict[str, Any]) -> ValidationResult:
...
def check_action_precondition(target: BaseModel, context: Dict[str, Any]) -> ValidationResult:
...
def check_dur_action_conditions(target: BaseModel, context: Dict[str, Any]) -> ValidationResult:
...
def check_dur_action_effect(target: BaseModel, context: Dict[str, Any]) -> ValidationResult:
...
def check_component_variables(target: BaseModel, context: Dict[str, Any]) -> ValidationResult:
    ...
    if hasattr(target, 'preconditions'):
        _check_variables_and_types(getattr(target, 'preconditions'), base_vars, sem, result, f"{model_type} '{name}' preconditions")
    elif hasattr(target, 'conditions'): # DurativeActions
        _check_variables_and_types(getattr(target, 'conditions'), base_vars, sem, result, f"{model_type} '{name}' conditions")
    elif hasattr(target, 'condition'): # DerivedPredicates
        _check_variables_and_types(getattr(target, 'condition'), base_vars, sem, result, f"{model_type} '{name}' condition")

    # Check Effects
    if hasattr(target, 'effects'):
        _check_variables_and_types(getattr(target, 'effects'), base_vars, sem, result, f"{model_type} '{name}' effects")

    # Check Duration
    if hasattr(target, 'duration'):
        dur_vars = dict(base_vars)
        dur_vars["?duration"] = "number"
        _check_variables_and_types(getattr(target, 'duration'), dur_vars, sem, result, f"{model_type} '{name}' duration")

    return result

def _check_variables_and_types(item: Any, var_types: Dict[str, str], sem: DomainSemantics, result: ValidationResult, location_desc: str):
            ...
            if sym in sem.signatures:
                ...
                for i, (arg, exp_type) in enumerate(zip(args, exp_types), 1):
                    ...
                    if arg_lower.startswith('?'):
                        ...
                        if not arg_type:
                            result.add_error(f"[ERROR] {location_desc}: '{arg}' is not a recognized variable, number, or constant.")
                            continue
