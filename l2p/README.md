...
Each template follows the same structure (Role, Output Format with XML tags, Rules, Task) with added cross-reference constraints - e.g., every predicate used in actions must be defined in the predicates section. Use them with `formalize_component()` by passing a **list of component classes** and the `prompt_template` argument:

```python
from l2p.utils.pddl_prompt import load_custom_template

prompt = load_custom_template(filepath="my_prompt.md")

results, raw = db.formalize_component(
    model=llm,
    component_class=[Predicate], # extract list of predicates
