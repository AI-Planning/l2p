...
class VLLM:
    ...
    def make_sft_prompt(self, system_message, prompt):
    ...
    def make_prompt(self, system_message, prompt):
        ...
        if 'llama-3' in self.model.lower() or 'llama3' in self.model.lower():
            full_prompt = prompt_templates["llama3"].format(system_prompt=system_message, prompt=prompt)
            full_prompt = full_prompt.strip()
        elif "codellama-13b" in self.model.lower():
        ...
        elif any([_ in self.model.lower() for _ in ["llama", 'agentlm']]): # llama prompt
