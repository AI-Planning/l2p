...
Examples:
  ...
  l2p config validate
    ...
    show_parser.add_argument(
        "--raw", action="store_true", help="Show raw YAML without formatting"
    )
...
def config_edit_command(args):
    ...
    if not editor:
        ...
        for candidate in ["nano", "vim", "vi", "code", "subl"]:
            if (
                subprocess.run(["which", candidate], capture_output=True).returncode
                == 0
            ):
                editor = candidate
                break
    ...
    try:
        ...
        if suggested_backend:
            ...
            print(
                f"(i) Auto-updated backend from '{current_backend}' to '{suggested_backend}' to match config_path"
            )
        ...
        raise CLIError(
            ...
            [
                ...
                "Ensure editor is properly installed",
            ],
        )
    except Exception as e:
        raise CLIError(
    ...
    try:
        ...
        raise CLIError(
            ...
            [
                ...
                "Try removing config file manually: rm {config_file}",
            ],
        )
...
def config_validate_command(args):
    ...
    checks = [
        ...
        (
            "Config Path",
            model_config.get("config_path"),
            bool(model_config.get("config_path")),
        ),
        (
            "API Key",
            "Set" if model_config.get("api_key") else "Missing",
            bool(model_config.get("api_key")),
        ),
    ]
