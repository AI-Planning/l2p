    ...
    def formalize_component(
        ...
        **ctx_kwargs
        ...
        if not prompt_template:
            if is_multi_class:
                raise ValueError(f"[ERROR] Must provide prompt template for multiple class extraction.")
            
            # retrieve default template
            else:
                cls = classes[0]
                primary_tag = cls.tag if isinstance(cls.tag, str) else cls.tag[0]
                prompt_template = getattr(DEF_DOMAIN_PROMPTS, primary_tag, 
        ...
        for attempt in range(max_retries):
            try:
                ...
                for cls in classes:
                    ...
                    for t in tags:
                        if raw_blocks := parse_xml_tags(llm_output=llm_output, tag_name=t):
                            extracted_results[cls] = parse_component(raw_blocks=raw_blocks, model_class=cls, tag_name=t)
                            break
                    else:
        ...
        def check_logical_condition(condition: LogicalCondition):
            ...
            if isinstance(condition, dict):
                ...
                if "preference" in condition:
                    reqs.add(":preferences")
                if op in ["always", "sometime", "at-most-once", "within", "hold-after", 
                          "hold-during", "sometime-after", "sometime-before", "always-within"]:
                    reqs.add(":constraints")
