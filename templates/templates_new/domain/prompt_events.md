...
## RULES
1. The JSON block above is strictly an ILLUSTRATIVE EXAMPLE. Do not copy names like "battery-depleted" or "rover" 
    unless they are explicitly defined in the domain description. You must extract actual events, variables, preconditions, and effects from the text.
...
5. **Preconditions Object:** Must be a dictionary containing a "conditions" array and an optional "desc". The "conditions" array represents the exact state that automatically triggers the event. 
    You can use plain strings for simple predicates/numeric comparisons, or logic dictionaries (e.g., `{"operator": "and", "conditions": [...]}`).
